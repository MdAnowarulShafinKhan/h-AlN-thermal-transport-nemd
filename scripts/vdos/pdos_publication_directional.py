...
Includes both species-projected spectra and Cartesian directional VDOS:
    x component = vibration along the Cartesian x direction
    y component = vibration along the Cartesian y direction
    z component = out-of-plane vibration along the Cartesian z direction
    total       = x + y + z

For strained 2D structures, x may be interpreted as the armchair/strain
direction and y as the in-plane transverse direction ONLY if the simulation
cell is oriented so that those crystallographic directions coincide with the
LAMMPS Cartesian x and y axes.
...
8. Computes Cartesian x-, y-, and z-resolved VDOS from vx, vy, and vz, respectively.
9. Enforces total VDOS = x + y + z at every frequency.
10. Normalizes the total integrated spectrum to 3 * primitive_atoms using one common scale factor,
    so directional components are never independently renormalized.
11. Uses only complete equal-length blocks; an incomplete final block is skipped.
...
Output files
------------
    <prefix>_THz_3N_raw.dat                         # total + species projections
    <prefix>_THz_3N_smoothed.dat                    # total + species projections
    <prefix>_THz_3N_directional_raw.dat             # total + x + y + z
    <prefix>_THz_3N_directional_smoothed.dat        # total + x + y + z
    <prefix>_metadata.txt
    <prefix>_total_pub.png / .pdf
    <prefix>_components_pub.png / .pdf              # species projections
    <prefix>_directional_pub.png / .pdf             # total + x + y + z
"""
...
def process_block(
...
) -> Tuple[np.ndarray, Dict[str, np.ndarray], "OrderedDict[str, np.ndarray]"]:
    """
    Return total, species-resolved, and Cartesian directional periodograms.

    The flattened DOF ordering is [x1,y1,z1,x2,y2,z2,...], so:
      x directional VDOS uses columns 0::3,
      y directional VDOS uses columns 1::3,
      z directional VDOS uses columns 2::3.

    The directional spectra are computed from the same mass-weighted,
    sampling-corrected periodogram as the species projections. Therefore
    P_total = P_x + P_y + P_z by construction (up to floating-point roundoff).
    """
    ...
    # Correct for type-wise atom sampling, if used. The factor is repeated
    # over x/y/z DOFs, so both species and directional sums remain unbiased.
    ...
    P_axes: "OrderedDict[str, np.ndarray]" = OrderedDict([
        ("x", P[:, 0::3].sum(axis=1)),
        ("y", P[:, 1::3].sum(axis=1)),
        ("z", P[:, 2::3].sum(axis=1)),
    ])
    return P_total, P_parts, P_axes
...
def main() -> None:
    ...
    natoms, ids, types, _first_timestep = read_first_snapshot(args.dump)
    ...
    acc_axes: Dict[str, np.ndarray] = OrderedDict()
    processed_blocks = 0
    filled = 0
    dump_frame_index = -1
    ...
    with open(args.dump, "r") as fh:
        ...
        while line:
            ...
            if filled == args.block_len:
                p_total, p_parts, p_axes = process_block(
                    data_block=block,
                    window=window,
                ...
                if acc_total is None:
                    ...
                    acc_axes = OrderedDict((axis, p_axes[axis].copy()) for axis in ("x", "y", "z"))
                else:
                    acc_total += p_total
                    for label in labels:
                        acc_parts[label] += p_parts[label]
                    for axis in ("x", "y", "z"):
                        acc_axes[axis] += p_axes[axis]
                processed_blocks += 1
                filled = 0
    ...
    if filled > 0:
        # Deliberately skip the final incomplete block. Mixing a shorter block would
        # introduce a different frequency grid, while zero-padding would change the
        # estimator. Publication runs should therefore use only full equal-length blocks.
        print(
            f"[info] Skipped final incomplete block: {filled}/{args.block_len} frames. "
            "This preserves a single frequency grid and avoids zero-padding bias.",
            file=sys.stderr,
        )
    ...
    s_axes = OrderedDict((axis, acc_axes[axis] / float(processed_blocks)) for axis in ("x", "y", "z"))
    ...
    # 3N normalization. Use ONE common scale from the total spectrum.
    # Never normalize x/y/z independently: their relative integrated weights are physical.
    ...
    g_axes = OrderedDict((axis, s_axes[axis] * scale) for axis in ("x", "y", "z"))

    # Enforce the requested directional definition exactly: Total = x + y + z.
    g_total = g_axes["x"] + g_axes["y"] + g_axes["z"]
    g_total[0] = 0.0
    for label in labels:
        g_parts[label][0] = 0.0
    for axis in ("x", "y", "z"):
    ...
    species_sum_raw = np.zeros_like(g_total)
    for label in labels:
        species_sum_raw += g_parts[label]
    axis_sum_raw = g_axes["x"] + g_axes["y"] + g_axes["z"]
    add_err_species_raw = float(np.max(np.abs(g_total - species_sum_raw)))
    add_err_direction_raw = float(np.max(np.abs(g_total - axis_sum_raw)))

    # Smooth species and directional components with the same Gaussian kernel.
    # Then rebuild total from x+y+z and apply one common final 3N scale.
    ...
    g_axes_s = OrderedDict((axis, gaussian_smooth(g_axes[axis], sigma_bins)) for axis in ("x", "y", "z"))
    ...
    for axis in ("x", "y", "z"):
        g_axes_s[axis][0] = 0.0
        g_axes_s[axis] = np.maximum(g_axes_s[axis], 0.0)

    g_total_s = g_axes_s["x"] + g_axes_s["y"] + g_axes_s["z"]

    area_s = _trapz(g_total_s, freqs_thz)
    if area_s <= 0:
        fail("Smoothed total spectral area is non-positive. Check smoothing/settings.")
    scale_s = target_area / area_s
    g_total_s *= scale_s
    for label in labels:
        g_parts_s[label] *= scale_s
    for axis in ("x", "y", "z"):
        g_axes_s[axis] *= scale_s

    species_sum_s = np.zeros_like(g_total_s)
    for label in labels:
        species_sum_s += g_parts_s[label]
    axis_sum_s = g_axes_s["x"] + g_axes_s["y"] + g_axes_s["z"]
    add_err_species_s = float(np.max(np.abs(g_total_s - species_sum_s)))
    add_err_direction_s = float(np.max(np.abs(g_total_s - axis_sum_s)))

    # Directional integrated weights after the common 3N normalization.
    # Their sum is 3*primitive_atoms; each component is NOT forced to primitive_atoms.
    area_x = _trapz(g_axes_s["x"], freqs_thz)
    area_y = _trapz(g_axes_s["y"], freqs_thz)
    area_z = _trapz(g_axes_s["z"], freqs_thz)

    # Save species-projected raw and smoothed data (backward-compatible filenames).
    ...
    raw_header = (
        ...
        "total_equals_sum_of_species=True"
    )
    ...
    sm_header = (
        ...
        f"primitive_atoms={args.primitive_atoms}; total_equals_sum_of_species=True"
    )
    np.savetxt(f"{args.prefix}_THz_3N_smoothed.dat", np.column_stack(sm_cols), header=sm_header, fmt="%.10e")

    # Save directional VDOS. x/y/z all share the same normalization scale, and Total=x+y+z.
    directional_raw_cols = [freqs_thz, g_total, g_axes["x"], g_axes["y"], g_axes["z"]]
    directional_raw_header = (
        "freq_THz  G_total  G_x  G_y  G_z\n"
        "Directional MD-derived VDOS: G_total = G_x + G_y + G_z at every frequency.\n"
        "G_x uses vx; G_y uses vy; G_z uses vz. Interpret x as armchair/strain and y as transverse "
        "only when those directions are aligned with the LAMMPS Cartesian axes.\n"
        f"mass_weighted={use_mass}; primitive_atoms={args.primitive_atoms}; "
        f"block_duration_ps={block_duration_ps:.8f}; df_THz={df_thz:.8f}"
    )
    np.savetxt(
        f"{args.prefix}_THz_3N_directional_raw.dat",
        np.column_stack(directional_raw_cols),
        header=directional_raw_header,
        fmt="%.10e",
    )

    directional_sm_cols = [freqs_thz, g_total_s, g_axes_s["x"], g_axes_s["y"], g_axes_s["z"]]
    directional_sm_header = (
        "freq_THz  G_total_s  G_x_s  G_y_s  G_z_s\n"
        "Smoothed directional MD-derived VDOS: G_total_s = G_x_s + G_y_s + G_z_s.\n"
        "G_x uses vx; G_y uses vy; G_z uses vz. A single common 3N normalization is applied.\n"
        f"Gaussian_sigma_THz={args.smooth_sigma_thz}; mass_weighted={use_mass}; "
        f"primitive_atoms={args.primitive_atoms}"
    )
    np.savetxt(
        f"{args.prefix}_THz_3N_directional_smoothed.dat",
        np.column_stack(directional_sm_cols),
        header=directional_sm_header,
        fmt="%.10e",
    )
    ...
    with open(f"{args.prefix}_metadata.txt", "w") as meta:
        ...
        meta.write(f"raw_species_additivity_error: {add_err_species_raw:.12e}\n")
        meta.write(f"smoothed_species_additivity_error: {add_err_species_s:.12e}\n")
        meta.write(f"raw_directional_additivity_error: {add_err_direction_raw:.12e}\n")
        meta.write(f"smoothed_directional_additivity_error: {add_err_direction_s:.12e}\n")
        meta.write("\nDirectional definition:\n")
        meta.write("x: Cartesian x velocity component vx; interpret as armchair/strain only if aligned with x\n")
        meta.write("y: Cartesian y velocity component vy; interpret as in-plane transverse only if aligned with y\n")
        meta.write("z: Cartesian z velocity component vz; out-of-plane\n")
        meta.write("total: x + y + z at every frequency\n")
        meta.write("directional components use one common 3N normalization; they are not normalized separately\n")
        meta.write(f"integral_Gx_smoothed: {area_x:.12g}\n")
        meta.write(f"integral_Gy_smoothed: {area_y:.12g}\n")
        meta.write(f"integral_Gz_smoothed: {area_z:.12g}\n")
        meta.write(f"integral_Gtotal_target: {target_area:.12g}\n")
        meta.write("\nAtom type summary:\n")
        for t in present_types:
            meta.write(
    ...
    # Directional VDOS plot. These are Cartesian components; x is the
    # armchair/strain direction only when the simulation cell is aligned accordingly.
    plt.figure(figsize=(7.0, 5.0))
    plt.plot(freqs_thz, g_total_s, label="Total = x + y + z", linewidth=1.6)
    plt.plot(freqs_thz, g_axes_s["x"], label="x (armchair/strain if aligned)", linewidth=1.2)
    plt.plot(freqs_thz, g_axes_s["y"], label="y (transverse if aligned)", linewidth=1.2)
    plt.plot(freqs_thz, g_axes_s["z"], label="z (out-of-plane)", linewidth=1.2)
    pub_axes()
    plt.xlim(args.xmin, args.xmax)
    plt.ylim(bottom=0)
    plt.xlabel("Frequency (THz)")
    plt.ylabel("Directional VDOS (states cell$^{-1}$ THz$^{-1}$)")
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(f"{args.prefix}_directional_pub.png", dpi=args.dpi)
    plt.savefig(f"{args.prefix}_directional_pub.pdf")
    plt.close()
    ...
    print(f"  max raw species additivity error: {add_err_species_raw:.3e}")
    print(f"  max smoothed species additivity error: {add_err_species_s:.3e}")
    print(f"  max raw directional additivity error (total-x-y-z): {add_err_direction_raw:.3e}")
    print(f"  max smoothed directional additivity error (total-x-y-z): {add_err_direction_s:.3e}")
    print(f"  directional integrated weights (smoothed): x={area_x:.6g}, y={area_y:.6g}, z={area_z:.6g}")
    print("  directional definition: x=vx, y=vy, z=vz; total=x+y+z")
